...
class MusicProcess:
            ...
            if match:
                ...
                if len(data) == 0:
                    ...
                    for idx, item in enumerate(data):
                        print(
                            f"[{idx}] [{item['score']}] [{item['artist']}] [{item['desc']}]"
                        )
                    try:
                        artist_index = int(
                            input("아티스트 선택 (번호:선택 그외:패스): ")
                        )
                ...
                for item in album_data:
                    ...
                    if move_flag == False:
                        ...
                        if item["title"] == album_tmp:
                            move_flag = True
                            logger.warning(
                                f"[!] [] 제외 일치 : [{album}] [{album_tmp}] [{item['title']}]"
                            )
                    ...
                    for item in ratio_album_data:
                        item["ratio"] = SequenceMatcher(
                            None, album.lower(), item["title"].lower()
                        ).ratio()
                    ratio_album_data = list(
                        (sorted(ratio_album_data, key=lambda k: k["ratio"]))
                    )
                    ...
                    def print_album(mode):
                        if mode.lower() == "r":
                            ...
                            for idx, item in enumerate(ratio_album_data):
                                print(
                                    f"[{idx}] {item['foldername']} [{int(item['ratio']*100)}%]"
                                )
                            print(
                                "====================================================="
                            )
                    ...
                    while True:
                        ans = input(
                            "앨범 선택 (번호:선택 D:시간순출력 R:유사도출력 L:파일목록, 번호L:트랙목록): "
                        )
                        list_flag = False
                        ans = ans.strip().lower()
                        if ans in ["d", "r"]:
                        ...
                        try:
                            ans = int(ans)
                            if list_flag:
                                tmp = SiteMelon.info_album(
                                    current_album_data[ans]["code"]
                                )
                                for _ in tmp["track"]:
                                    for __ in _:
                                        print(__["title"])
                                ...
                                ret = self.move(
                                    ...
                                    album,
                                    album_path,
                                    current_album_data[ans]["foldername"],
                                )
                                break
    ...
    def move(self, artist, album, album_path, new_album):
        try:
            ...
            new_artist_path = os.path.join(
                self.config["target"],
                SupportString.get_cate_char_by_first(artist),
                artist,
            )
    ...
    def remove_trash_file(self):
        for base, dirs, files in os.walk(self.config["remove_target"]):
            ...
            for f in files:
                ...
                if (
                    filename.lower()
                    in self.config["remove_except_filename_except_ext"]
                ):
                    print_current()
                    logger.warning(f"소문자로 변경 : {f}")
                    shutil.move(
                        os.path.join(base, f), os.path.join(base, f.lower())
                    )
                    continue
                if ext in self.config["remove_except_ext"]:
                    if ".com" in filename:
                        ...
                        if f != tmp:
                            shutil.move(
                                os.path.join(base, f), os.path.join(base, tmp)
                            )
                            continue
                ...
                if ext.lower() in self.config["remove_except_ext"]:
                    ...
                    shutil.move(
                        os.path.join(base, f),
                        os.path.join(base, f"{filename}.{ext.lower()}"),
                    )
                    continue
    ...
    def process_cli(cls):
        ...
        parser.add_argument(
            "--config", default="config.yaml", help="config filepath"
        )
